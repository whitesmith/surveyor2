...
describe Surveyor::Parser do
  ...
  context 'basic questions' do
    ...
    it 'parses' do
      ...
      expect(Surveyor::Question.count).to eq(4)
      expect(Surveyor::Answer.count).to eq(9)
      ...
      surveys.each { |attrs| attrs.each { |k, v| expect(Surveyor::Survey.where(display_order: attrs[:display_order]).first[k]).to eq(v) } }
      sections.each { |attrs| attrs.each { |k, v| expect(Surveyor::SurveySection.where(display_order: attrs[:display_order]).first[k]).to eq(v) } }
      questions.each { |attrs| attrs.each { |k, v| expect(Surveyor::Question.where(display_order: attrs[:display_order]).first[k]).to eq(v) } }
      answers_1.each { |attrs| attrs.each { |k, v| expect(Surveyor::Question.where(display_order: 1).first.answers.where(display_order: attrs[:display_order]).first[k]).to eq(v) } }
      answers_2.each { |attrs| attrs.each { |k, v| expect(Surveyor::Question.where(display_order: 2).first.answers.where(display_order: attrs[:display_order]).first[k]).to eq(v) } }
    end
  end
  ...
  context 'complex questions' do
    ...
    it 'parses' do
      expect(Surveyor::Survey.count).to eq(1)
      expect(Surveyor::QuestionGroup.count).to eq(3)
      expect(Surveyor::Question.count).to eq(10)
      expect(Surveyor::Answer.count).to eq(34)
      ...
      grid_questions = Surveyor::QuestionGroup.where(reference_identifier: 'today').first.questions
      ...
      surveys.each { |attrs| attrs.each { |k, v| expect(Surveyor::Survey.where(display_order: attrs[:display_order]).first[k]).to eq(v) } }
      question_groups.each { |attrs| attrs.each { |k, v| expect(Surveyor::QuestionGroup.where(reference_identifier: attrs[:reference_identifier]).first[k]).to eq(v) } }
      ...
      dropdown_question_attributes.each { |k, v| expect(Surveyor::Question.where(display_type: 'dropdown').first[k]).to eq(v) }
    end
  end
  ...
  context 'depdencies and validations' do
    ...
    it 'parses' do
      ...
      dependency_conditions.each { |attrs| expect(Surveyor::DependencyCondition.where(rule_key: attrs[:rule_key]).first.question.reference_identifier).to eq(attrs[:question_reference_identifier]) }
      dependency_conditions.each { |attrs| (expect(Surveyor::DependencyCondition.where(rule_key: attrs[:rule_key]).first.answer.reference_identifier).to eq(attrs[:answer_reference_identifier])) if attrs[:answer_reference_identifier] }
    end
  end
  ...
  context 'translations' do
    it 'should produce the survey text for :default locale' do
      ...
      expect(survey.is_a?(Surveyor::Survey)).to eq(true)
    ...
    it 'should raise an error w/o :default locale' do
      ...
      s = Surveyor::Survey.all.size
      expect { Surveyor::Parser.new.parse(survey_text) }.to raise_error(Surveyor::ParserError)
      expect(Surveyor::Survey.all.size).to eq(s)
    end
        ...
        it 'should look for the translation file relative to pwd' do
          Dir.mktmpdir do |dir|
            FileUtils.cd(dir) do
              ...
              survey = Surveyor::Survey.where(title: 'One language is never enough').first
  ...
  context 'quizzes' do
    ...
    it 'parses' do
      expect(Surveyor::Survey.count).to eq(1)
      expect(Surveyor::SurveySection.count).to eq(2)
      expect(Surveyor::Question.count).to eq(3)
      expect(Surveyor::Answer.count).to eq(11)
      ...
      surveys.each { |attrs| attrs.each { |k, v| expect(Surveyor::Survey.where(display_order: attrs[:display_order]).first[k]).to eq(v) } }
      sections.each { |attrs| attrs.each { |k, v| expect(Surveyor::SurveySection.where(display_order: attrs[:display_order]).first[k]).to eq(v) } }
      questions.each { |attrs| attrs.each { |k, v| expect(Surveyor::Question.where(reference_identifier: attrs[:reference_identifier]).first[k]).to eq(v) } }
      answers_1.each { |attrs| attrs.each { |k, v| expect(Surveyor::Question.where(reference_identifier: '1').first.answers.where(display_order: attrs[:display_order]).first[k]).to eq(v) } }
      answers_2.each { |attrs| attrs.each { |k, v| expect(Surveyor::Question.where(reference_identifier: '2').first.answers.where(display_order: attrs[:display_order]).first[k]).to eq(v) } }
      correct_answers.each { |attrs| expect(Surveyor::Question.where(reference_identifier: attrs[:question_reference_identifier]).first.correct_answer.text).to eq(attrs[:correct_answer_text]) }
    end
  end
  ...
  context 'mandatory' do
    it 'parses' do
      ...
      expect(Surveyor::Survey.count).to eq(1)
      expect(Surveyor::SurveySection.count).to eq(1)
      expect(Surveyor::Question.count).to eq(3)
