...
describe Surveyor::ResponseSet, type: :model do
  subject(:response_set) { FactoryBot.create(:response_set) }
  ...
  context 'access_code' do
    ...
    it 'should be accepted in constructor' do
      response_set = FactoryBot.build(:response_set)
  ...
  describe 'with dependencies' do
    let!(:section) { FactoryBot.create(:survey_section) }
    # Questions
    let!(:do_you_like_pie) { FactoryBot.create(:question, text: 'Do you like pie?', survey_section: section) }
    let!(:what_flavor) { FactoryBot.create(:question, text: 'What flavor?', survey_section: section) }
    let!(:what_bakery) { FactoryBot.create(:question, text: 'What bakery?', survey_section: section) }
    # Answers
    let!(:do_you_like_pie_answer_yes) { FactoryBot.create(:answer, text: 'yes', question_id: do_you_like_pie.id) }
    let!(:do_you_like_pie_answer_no) { FactoryBot.create(:answer, text: 'no', question_id: do_you_like_pie.id) }
    let!(:what_flavor_answer) { FactoryBot.create(:answer, response_class: :string, question_id: what_flavor.id) }
    let!(:what_bakery_answer) { FactoryBot.create(:answer, response_class: :string, question_id: what_bakery.id) }
    # Dependency
    let!(:what_flavor_dep) { FactoryBot.create(:dependency, rule: 'A', question_id: what_flavor.id) }
    let!(:what_flavor_dep_cond) { FactoryBot.create(:dependency_condition, rule_key: 'A', question_id: do_you_like_pie.id, operator: '==', answer_id: do_you_like_pie_answer_yes.id, dependency_id: what_flavor_dep.id) }
    let!(:what_bakery_dep) { FactoryBot.create(:dependency, rule: 'B', question_id: what_bakery.id) }
    let!(:what_bakery_dep_cond) { FactoryBot.create(:dependency_condition, rule_key: 'B', question_id: do_you_like_pie.id, operator: '==', answer_id: do_you_like_pie_answer_yes.id, dependency_id: what_bakery_dep.id) }
    # Responses
    subject(:response_set) { FactoryBot.create(:response_set) }
    let!(:response_1) { FactoryBot.create(:response, question_id: do_you_like_pie.id, answer_id: do_you_like_pie_answer_yes.id, response_set_id: response_set.id) }
    let!(:response_2) { FactoryBot.create(:response, string_value: 'pecan pie', question_id: what_flavor.id, answer_id: what_flavor_answer.id, response_set_id: response_set.id) }
    ...
    it 'should list group as dependency' do
      ...
      FactoryBot.create(:dependency_condition, rule_key: 'C', question_id: do_you_like_pie.id, operator: '==', answer_id: do_you_like_pie_answer_yes.id, dependency_id: crust_group_dep.id)
  ...
  describe 'dependency_conditions' do
    let!(:section) { FactoryBot.create(:survey_section) }
    # Questions
    let!(:like_pie) { FactoryBot.create(:question, text: 'Do you like pie?', survey_section: section) }
    let!(:like_jam) { FactoryBot.create(:question, text: 'Do you like jam?', survey_section: section) }
    let!(:what_is_wrong_with_you) { FactoryBot.create(:question, text: "What's wrong with you?", survey_section: section) }
    # Answers
    let!(:like_pie_yes) { FactoryBot.create(:answer, text: 'yes', question_id: like_pie.id) }
    let!(:like_pie_no) { FactoryBot.create(:answer, text: 'no', question_id: like_pie.id) }
    let!(:like_jam_yes) { FactoryBot.create(:answer, text: 'yes', question_id: like_jam.id) }
    let!(:like_jam_no) { FactoryBot.create(:answer, text: 'no', question_id: like_jam.id) }
    # Dependency
    let!(:what_is_wrong_with_you_dep) { FactoryBot.create(:dependency, rule: 'A or B', question_id: what_is_wrong_with_you.id) }
    let!(:dep_a) { FactoryBot.create(:dependency_condition, rule_key: 'A', question_id: like_pie.id, operator: '==', answer_id: like_pie_yes.id, dependency_id: what_is_wrong_with_you_dep.id) }
    let!(:dep_b) { FactoryBot.create(:dependency_condition, rule_key: 'B', question_id: like_jam.id, operator: '==', answer_id: like_jam_yes.id, dependency_id: what_is_wrong_with_you_dep.id) }
    # Responses
    subject(:response_set) { FactoryBot.create(:response_set) }
    let!(:response) { FactoryBot.create(:response, question_id: like_pie.id, answer_id: like_pie_yes.id, response_set_id: response_set.id) }
    ...
    it 'should list all dependencies for passed question_id' do
      ...
      FactoryBot.create(:dependency_condition, rule_key: 'A', question_id: like_ice_cream.id, operator: '==', answer_id: like_ice_cream.answers.first.id, dependency_id: flavor_dependency.id)
      # Responses
  ...
  describe 'as a quiz' do
    ...
    subject(:response_set) { FactoryBot.create(:response_set, survey: survey) }
    ...
    def generate_responses(count, mandatory = nil, dependent = nil, triggered = nil)
      ...
      count.times do |_i|
        ...
        if dependent == 'dependent'
          ...
          FactoryBot.create(:dependency_condition, dependency: d, question_id: dq.id, answer_id: da.id)
        end
