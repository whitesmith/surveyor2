...
module Surveyor
  ...
  class Parser
    ...
    def method_missing(missing_method, *args, &block)
      ...
      Surveyor::Parser.raise_error("\"#{type}\" is not a surveyor method.") unless models.include?(type)

      Surveyor::Parser.rake_trace(reference_identifier.blank? ? "#{type} #{args.map(&:inspect).join(', ')}" : "#{type}_#{reference_identifier} #{args.map(&:inspect).join(', ')}", block_models.include?(type) ? 2 : 0)
  ...
  def parse_and_build(context, args, original_method, reference_identifier)
    ...
    if context[:question_group] && context[:question_group].display_type == "grid"
      (context[:grid_answers] || []).each do |grid_answer|
        a = context[:question].answers.build(Surveyor::PermittedParams.new(grid_answer.attributes.reject { |k,v| %w(id api_id created_at updated_at).include?(k) }).answer)
        unless self.reference_identifier.blank?
...
module SurveyorParserAnswerMethods
  def parse_and_build(context, args, original_method, reference_identifier)
    ...
    # add answers to grid
    if context[:question_group] && context[:question_group].display_type == "grid"
      self.attributes = Surveyor::PermittedParams.new({ display_order: context[:grid_answers].size }.merge(attrs)).answer
      context[:grid_answers] << context[:answer] = self
    else
      self.attributes = Surveyor::PermittedParams.new({ display_order: context[:question].answers.size }.merge(attrs)).answer
...
module SurveyorParserValidationMethods
  def parse_and_build(context, args, original_method, reference_identifier)
    ...
    # build and set context
    self.attributes = Surveyor::PermittedParams.new({ rule: "A" }.merge(args[0] || {})).validation
