...
child :sections => :sections do
  ...
  child :questions_and_groups => :questions_and_groups do
    ...
    attribute :api_id                   => :uuid
    node(:text,                     :if => lambda { |q| q.is_a?(Surveyor::Question)}){ |q| q.split(q.text, :pre) }
    ...
    node(:post_text,                :if => lambda { |q| q.is_a?(Surveyor::Question) && !q.split(q.text, :post).blank? }){ |q| q.split(q.text, :post) }
